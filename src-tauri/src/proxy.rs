...
#[link(name = "wininet")]
extern "system" {
    fn InternetSetOptionW(h: *const std::ffi::c_void, option: u32, buffer: *const std::ffi::c_void, len: u32) -> i32;
}
...
pub fn set_system_proxy_inner(enable: bool, port: u16) {
    if std::env::consts::OS != "windows" { return; }
    let _ = Command::new("reg")
        .args(["add", PROXY_KEY, "/v", "ProxyEnable", "/t", "REG_DWORD", "/d", if enable { "1" } else { "0" }, "/f"])
        .output();
    if enable && port > 0 {
        let _ = Command::new("reg")
            .args(["add", PROXY_KEY, "/v", "ProxyServer", "/t", "REG_SZ", "/d", &format!("127.0.0.1:{}", port), "/f"])
            .output();
    }
    // 通知系统刷新，立即生效
